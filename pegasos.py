'''
Created on Apr 12, 2017
Sequential Pegasos 
the input T is k*T in Batch Pegasos
@author: shiqi bao
'''
from numpy import *


def loadDataSet(fileName):
    dataMat = [];
    labelMat = []
    fr = open(fileName)
    for line in fr.readlines():
        lineArr = line.strip().split('\t')
        # dataMat.append([float(lineArr[0]), float(lineArr[1]), float(lineArr[2])])
        dataMat.append([float(lineArr[0]), float(lineArr[1])])
        labelMat.append(float(lineArr[2]))
    return dataMat, labelMat


def seqPegasos(dataSet, labels, lam, T):
    m, n = shape(dataSet);
    w = zeros(n)
    for t in range(1, T + 1):
        i = random.randint(m)
        eta = 1.0 / (lam * t)
        p = predict(w, dataSet[i, :])
        if labels[i] * p < 1:
            w = (1.0 - 1 / t) * w + eta * labels[i] * dataSet[i, :]
        else:
            w = (1.0 - 1 / t) * w
        #print w
    return w


def predict(w, x):
    return w * x.T


def batchPegasos(dataSet, labels, lam, T, k):
    m, n = shape(dataSet);
    w = zeros(n);
    dataIndex = range(m)
    for t in range(1, T + 1):
        wDelta = mat(zeros(n))  # reset wDelta
        eta = 1.0 / (lam * t)
        random.shuffle(dataIndex)
        for j in range(k):  # go over training set
            i = dataIndex[j]
            p = predict(w, dataSet[i, :])  # mapper code
            if labels[i] * p < 1:  # mapper code
                wDelta += labels[i] * dataSet[i, :].A  # accumulate changes
        w = (1.0 - 1 / t) * w + (eta / k) * wDelta  # apply changes at each T
    return w

def sklearn_svm_smo(dataSet, labels):
    from sklearn import svm
    clf = svm.SVC(kernel='linear',max_iter=1000,C=0.5)
    clf.fit(dataSet,labels)
    return clf.coef_

#import data
datArr, labelList = loadDataSet('testSet.txt')
datMat = mat(datArr)

#runtime
import time
start = time.time()
finalWs = seqPegasos(datMat, labelList, 2, 1000)
end = time.time()
print 'pegasos time', end-start
# finalWs = batchPegasos(datMat, labelList, 2, 50, 100)
print finalWs

start = time.time()
weight = sklearn_svm_smo(datMat, labelList)
end = time.time()
print 'smo time' ,end-start
print weight

#plot results
import matplotlib.pyplot as plt

fig = plt.figure()
ax = fig.add_subplot(111)
x1 = []
y1 = []
xm1 = []
ym1 = []
for i in range(len(labelList)):
    if labelList[i] == 1.0:
        x1.append(datMat[i, 0])
        y1.append(datMat[i, 1])
    else:
        xm1.append(datMat[i, 0])
        ym1.append(datMat[i, 1])
ax.scatter(x1, y1, marker='s', s=90)
ax.scatter(xm1, ym1, marker='o', s=50, c='red')
x = arange(-6.0, 8.0, 0.1)
y = (-finalWs[0, 0] * x - 0) / finalWs[0, 1]
y1 = (-weight[0, 0] * x - 0) / weight[0, 1]
# y2 = (0.43799*x)/0.12316weight
y2 = (0.498442 * x) / 0.092387  # 2 iterations
ax.plot(x, y)
ax.plot(x, y1, 'g-.')
ax.axis([-6, 8, -4, 5])
ax.legend(('pegasos', 'smo'))
plt.show()